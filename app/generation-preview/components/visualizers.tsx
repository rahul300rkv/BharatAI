...
function PdfScanVisualizer() {
  return (
    <div className="size-32 relative flex items-center justify-center">
      ...
      />
      <div className="w-20 h-28 bg-card dark:bg-card rounded-lg border border-muted200 dark:border-border 700 shadow-xl relative overflow-hidden">
        <div className="p-3 space-y-2 mt-1">
          {[80, 60, 90, 45, 70].map((w, i) => (
            <motion.div
              key={i}
              className="h-1.5 bg-muted dark:bg-card rounded"
              style={{ width: `${w}%` }}
...
function WebSearchVisualizer({ sources }: { sources: Array<{ title: string; url: string }> }) {
  ...
  return (
    <div className="size-56 relative flex items-center justify-center">
      ...
      {/* Search results card */}
      <div className="w-44 bg-card dark:bg-card rounded-xl border border-muted200 dark:border-border 700 shadow-xl overflow-hidden relative">
        {/* Search bar header */}
        <div className="px-3 py-2 border-b border-border dark:border-borderte-700 flex items-center gap-2">
          <Search className="size-3 text-teal-500 shrink-0" />
          <div className="flex-1 h-4 bg-muted/40 dark:bg-card/50 rounded-full overflow-hidden flex items-center px-2">
            <motion.div
            ...
            ? // Skeleton: pulsing result placeholders
              skeletonResults.map((item, i) => (
                ...
                >
                  ...
                  />
                  <div
                    className="h-1 bg-muted dark:bg-card rounded"
              ...
              sources.slice(0, 4).map((source, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      <div className="h-0.5 flex-1 bg-muted dark:bg-card rounded-full" />
                      <div className="h-0.5 w-1/3 bg-muted dark:bg-card rounded-full" />
                    </div>
                  </motion.div>
                );
              })}
...
function StreamingOutlineVisualizer({ outlines }: { outlines: SceneOutline[] }) {
  ...
  return (
    <div className="w-40 h-52 bg-card dark:bg-card rounded-xl border border-muted200 dark:border-border 700 shadow-2xl p-4 overflow-hidden relative rotate-[-2deg] hover:rotate-0 transition-transform duration-500">
      <div className="absolute top-0 inset-x-0 h-1 bg-primary/50" />
      <div className="w-1/3 h-2 bg-muted dark:bg-card rounded mb-3" />
      ...
      <motion.div
        className="absolute bottom-3 right-3 size-2 bg-primary rounded-full"
    ...
    <div className="w-60 h-40 mx-auto flex items-center justify-center">
      <div className="flex gap-3">
        {[0, 1, 2].map((i) => (
          <motion.div
            key={i}
            className="w-14 h-20 rounded-lg bg-gradient-to-br from-primary/200 to-blue-500 dark:from-primary/300 dark:to-blue-700 shadow-lg"
    ...
    switch (idx) {
      case 0:
        return {
          ...
          label: 'SLIDE',
          badge:
            'bg-primary/100 text-primary 600 border-primary/200 dark:bg-primary/900/40 dark:text-primary 300 dark:border-primary/800',
        };
      case 1:
        return {
          color: 'purple',
          label: 'QUIZ',
          badge:
            'bg-primary/12 text-primary border-primary/100 dark:bg-primary/450/40 dark:text-primary/150 dark:border-primary/400',
        };
      case 2:
        return {
          color: 'amber',
    ...
    <div className="size-56 relative flex items-center justify-center perspective-[800px]">
      ...
      <motion.div
        ...
        className={cn(
          ...
          theme.color === 'purple' && 'bg-primary/80/10',
      ...
      {[0, 1].map((i) => (
        <motion.div
          ...
          className={cn(
            ...
            theme.color === 'blue' && 'border-primary/500/10',
            theme.color === 'purple' && 'border-primary/250/10',
      ...
      <div className="w-40 h-28 relative">
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className={cn(
              'absolute inset-0 bg-card dark:bg-card rounded-xl border shadow-xl overflow-hidden flex flex-col p-3 origin-center',
              theme.color === 'blue' && 'border-primary/200 dark:border-primary/900/30',
              theme.color === 'purple' && 'border-primary/100 dark:border-primary/450/30',
          ...
          >
                  ...
                  <div className="flex-1 space-y-2">
                    {[0.8, 0.9, 0.6, 0.7].map((w, i) => (
                      <motion.div
                        ...
                        className="h-1.5 bg-muted dark:bg-card rounded-full"
                      />
                    ))}
                  </div>
                  <motion.div
                    ...
                    className="w-12 h-12 bg-primary/50 dark:bg-primary/900/20 rounded-lg flex items-center justify-center shrink-0"
                  >
                    <BarChart3 className="size-6 text-primary 500/60" />
                  </motion.div>
                </div>
              </div>
            )}
            ...
            {index === 1 && (
              <div className="flex flex-col h-full justify-center space-y-2 pt-2">
                ...
                >
                  <div className="h-2 w-3/4 bg-primary/80/20 rounded-full" />
                </motion.div>
                  ...
                  {[0, 1, 2, 3].map((i) => (
                    <motion.div
                      ...
                      className={cn(
                        ...
                        i === 1
                          ? 'bg-primary/80 text-white border-primary/250'
                          : 'bg-muted/40 dark:bg-card/50 border-border dark:border-borderte-700',
                      )}
                    >
                      <div
                        className={cn(
                          'size-1.5 rounded-full mr-2',
                          i === 1 ? 'bg-card' : 'bg-muted',
                        )}
                      />
                      <div
                        className={cn(
                          'h-1 w-8 rounded-full',
                          i === 1 ? 'bg-card/50' : 'bg-muted dark:bg-card',
                        )}
                      />
                    </motion.div>
                  ))}
                ...
                <div className="flex-1 flex gap-2 overflow-hidden">
                  {[0, 1, 2].map((col) => (
                    <motion.div
                      ...
                      className="flex-1 bg-muted/40 dark:bg-card/30 rounded flex flex-col gap-1 p-1"
                    >
                      <div className="h-1 w-6 bg-muted dark:bg-card rounded mb-1" />
                      {[0, 1].map((card) => (
                        <div
                          key={card}
                          className="h-3 w-full bg-card dark:bg-card rounded border border-border dark:border-borderte-500 shadow-sm"
                        />
                      ))}
                    </motion.div>
                  ))}
            ...
            {index === 3 && (
              <div className="flex flex-col h-full relative pt-1">
                ...
                <div className="flex items-center gap-1 mb-2 border-b border-border dark:border-borderte-700 pb-1 pr-10">
                  <div className="flex gap-0.5">
                    <div className="size-1.5 rounded-full bg-red-400" />
                    <div className="size-1.5 rounded-full bg-amber-400" />
                    <div className="size-1.5 rounded-full bg-green-400" />
                  </div>
                  <div className="h-1.5 flex-1 bg-muted dark:bg-card rounded-full ml-1" />
                </div>
                ...
                <div className="flex-1 flex gap-2 relative">
                  <motion.div
                    ...
                    className="w-1/3 bg-muted/40 dark:bg-card/30 rounded p-1 space-y-1"
                  >
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-1 w-full bg-muted dark:bg-card rounded-full"
                      />
                    ))}
                  </motion.div>
...
function ActionsVisualizer() {
  ...
  const actionItems = [
    {
      ...
      label: 'Speech',
      color: 'text-primary',
      activeBg: 'bg-primary/80/10',
      activeBorder: 'border-primary/100 dark:border-primary/400',
    },
    {
      icon: Focus,
      label: 'Spotlight',
  ...
  return (
    <div className="size-56 relative flex items-center justify-center">
      {/* Background pulse */}
      <motion.div
        className="absolute inset-0 blur-3xl rounded-full bg-primary/80/8"
        animate={{ scale: [1, 1.15, 1], opacity: [0.3, 0.5, 0.3] }}
        transition={{ duration: 3.5, repeat: Infinity }}
      />
      ...
      <div className="w-44 bg-card dark:bg-card rounded-xl border border-muted200 dark:border-border 700 shadow-xl overflow-hidden relative">
        ...
        <div className="px-3 py-2 border-b border-border dark:border-borderte-700 flex items-center gap-2">
          ...
          <motion.div
            ...
            animate={{ width: '50%' }}
            transition={{ delay: 0.2 }}
            className="h-1.5 bg-primary/80/20 rounded-full"
          />
        </div>
        ...
        <div className="p-2 space-y-1.5 relative">
          ...
          <motion.div
            className="absolute left-2 right-2 rounded-lg bg-primary/80/[0.06] dark:bg-primary/200/[0.08]"
            style={{ height: ROW_H - 6 }}
              ...
              >
                <div
                  className={cn(
                    ...
                    isActive ? item.color : 'text-muted300 dark:text-muted-foreground',
                  )}
                >
                ...
                <div className="flex-1 flex items-center gap-1.5">
                  <span
                    className={cn(
                      ...
                      isActive ? item.color : 'text-muted-foreground dark:text-muted-foreground',
                    )}
                  >
                    {item.label}
                  </span>
                  <div
                    className={cn(
                      ...
                      isActive ? 'bg-current opacity-20' : 'bg-muted dark:bg-card',
                    )}
                  />
                </div>
