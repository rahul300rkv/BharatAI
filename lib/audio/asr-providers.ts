...
export async function transcribeAudio(
  ...
  switch (config.providerId) {
    ...
    case 'groq-whisper':
      return await transcribeGroqWhisper(config, audioBuffer);
 ...
 *
 * Groq runs whisper-large-v3 at extremely low latency via its LPU hardware.
 * The API is OpenAI-compatible, so we reuse createOpenAI with Groq's base URL.
 * Models: whisper-large-v3, whisper-large-v3-turbo
 */
async function transcribeGroqWhisper(
  config: ASRModelConfig,
  audioBuffer: Buffer | Blob,
): Promise<ASRTranscriptionResult> {
  // Groq transcription is OpenAI-compatible — use createOpenAI pointed at Groq
  const groqAsOpenAI = createOpenAI({
    apiKey: config.apiKey!,
    baseURL: config.baseUrl || ASR_PROVIDERS['groq-whisper'].defaultBaseUrl,
  });
  ...
  try {
    const result = await transcribe({
      model: groqAsOpenAI.transcription('whisper-large-v3-turbo'),
      audio: audioData,
      providerOptions: {
        openai: {
          language: config.language === 'auto' ? undefined : config.language,
