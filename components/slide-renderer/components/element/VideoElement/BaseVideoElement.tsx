...
export function BaseVideoElement({ elementInfo }: BaseVideoElementProps) {
  ...
  return (
    ...
    >
      ...
      >
        {showDisabled ? (
          <div className="w-full h-full bg-muted/40 dark:bg-[#1A0C00]/40 flex items-center justify-center rounded">
            <div className="flex items-center gap-1 px-2 py-1 text-[10px] font-medium text-muted-foreground dark:text-muted-foreground">
              <VideoOff className="w-3 h-3 shrink-0" />
              <span>{t('settings.mediaGenerationDisabled')}</span>
            </div>
          </div>
        ) : showSkeleton ? (
          <div className="w-full h-full bg-gradient-to-br from-primary/6 via-[#F5A800]/5 to-[#006B8F]/5 dark:from-indigo-950/40 dark:via-primary/475/30 dark:to-blue-950/20 flex items-center justify-center rounded">
            <style>{`
